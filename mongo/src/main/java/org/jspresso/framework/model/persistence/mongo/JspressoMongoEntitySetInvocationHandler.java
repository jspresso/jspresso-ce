 ...
 */
public class JspressoMongoEntitySetInvocationHandler implements InvocationHandler {

  private Collection<Serializable> ids;
  private Class<? extends IEntity> entityContract;
  private MongoTemplate            mongo;
  private Set<IEntity>             target;
   ...
   *     the mongo
   */
  public JspressoMongoEntitySetInvocationHandler(Collection<Serializable> ids, Class<? extends IEntity> entityContract,
                                                 MongoTemplate mongo) {
    this.ids = ids;
  ...
  private Object invokeTargetMethod(Method method, Object... args) throws NoSuchMethodException {
    if (target == null) {
      if (ids == null || ids.isEmpty()) {
        ...
        target = new LinkedHashSet<>(mongo.find(new Query(Criteria.where("_id").in(ids)), entityContract));
      }
    }
    try {
