...
import com.d2s.framework.binding.echo.ActionFieldConnector;
      ...
      if (viewDescriptor.getActions() != null) {
        ...
        for (Iterator<Map.Entry<String, List<IDisplayableAction>>> iter = viewDescriptor
          ...
          for (IDisplayableAction action : nextActionSet.getValue()) {
            Button actionButton = createButton(action, actionHandler, view,
                locale);
...
//    return view;
  ...
  private IView<Component> createReferencePropertyView(
      IReferencePropertyDescriptor propertyDescriptor,
      IActionHandler actionHandler, Locale locale) {
    ActionField viewComponent = createActionField(true);
    ActionFieldConnector connector = new ActionFieldConnector(
        propertyDescriptor.getName(), viewComponent);
    connector.setExceptionHandler(actionHandler);
    Button fieldButton = createButton(lovAction, actionHandler, viewComponent,
        propertyDescriptor, connector, locale);
    fieldButton.setToolTipText(getTranslationProvider().getTranslation(
        "lov.element.description",
        new Object[] {propertyDescriptor.getReferencedDescriptor().getI18nName(
            translationProvider, locale)}, locale)
        + TOOLTIP_ELLIPSIS);
    if (propertyDescriptor.getReferencedDescriptor().getIconImageURL() != null) {
      fieldButton.setIcon(iconFactory.getIcon(propertyDescriptor
          .getReferencedDescriptor().getIconImageURL(),
          IIconFactory.TINY_ICON_SIZE));
    }
    viewComponent.setActions(Collections.singletonList(fieldButton));
  ...
  private IView<Component> createBinaryPropertyView(
      ...
      IActionHandler actionHandler, Locale locale) {
    ActionField viewComponent = createActionField(false);
    ActionFieldConnector connector = new ActionFieldConnector(
        propertyDescriptor.getName(), viewComponent);
    connector.setExceptionHandler(actionHandler);
    Button openAction = createButton(openFileAsBinaryPropertyAction,
        actionHandler, viewComponent, propertyDescriptor, connector, locale);
    Button saveAction = createButton(saveBinaryPropertyAsFileAction,
        actionHandler, viewComponent, propertyDescriptor, connector, locale);
    Button resetAction = createButton(resetPropertyAction, actionHandler,
        viewComponent, propertyDescriptor, connector, locale);
    Button infoAction = createButton(binaryPropertyInfoAction, actionHandler,
        viewComponent, propertyDescriptor, connector, locale);
    viewComponent.setActions(Arrays.asList(new Button[] {openAction,
   ...
   */
  protected ActionField createActionField(boolean showTextField) {
    return new ActionField(showTextField);
  }
  ...
  private ButtonEx createButton(IDisplayableAction action,
      IActionHandler actionHandler, IView<Component> view, Locale locale) {
    ButtonEx actionButton = createButton();
    ...
    actionButton.addActionListener(echoAction);
    return actionButton;
  }

  private ButtonEx createButton(IDisplayableAction action,
      IActionHandler actionHandler, Component viewComponent,
      IModelDescriptor modelDescriptor, IValueConnector viewConnector,
      Locale locale) {
    ButtonEx actionButton = createButton();
    ActionListener echoAction = actionFactory.createAction(action,
        actionHandler, viewComponent, modelDescriptor, viewConnector, locale);
    actionButton.setIcon(iconFactory.getIcon(action.getIconImageURL(),
        IIconFactory.TINY_ICON_SIZE));
    String i18nDescription = action.getI18nDescription(translationProvider,
        locale);
